...
def classify_images(images_dir, results_dic, model):
    ...
    """
    filenames_list = list(results_dic.keys())
    classifier_labels = []

    for idx in range(0, len(filenames_list), 1):
        test_image = images_dir+filenames_list[idx]
        model_label = classifier(test_image, model)
        lower_model_label = model_label.lower()
        word_list_classifier_label = lower_model_label.split(",")
        label_name = ""
        done = None
        classifier_labels = []

        for word in word_list_classifier_label:
            ...
            if label_word == results_dic[filenames_list[idx]][0]:
                ...
                break
        if done != 1:
            results_dic[filenames_list[idx]].extend([classifier_labels[0],0])